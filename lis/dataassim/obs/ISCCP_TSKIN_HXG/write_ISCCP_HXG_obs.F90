...
!BOP
! 
! !ROUTINE: write_ISCCP_HXG_obs
! \label{write_ISCCP_HXG_stobs}
! 
! !REVISION HISTORY: 
! 25Jan2008: Sujay Kumar; Initial Specification
! 6 APR 2021: John Eylander; Modified for NOAA ISCCP data
! 
! !INTERFACE: 
subroutine write_ISCCP_HXG_obs(n, k, OBS_State)
! !USES: 
  use ESMF
  use LIS_coreMod
  use LIS_logMod
...
!EOP
  ...
  logical                  :: data_update
  real, pointer            :: stobs(:)
  real                     :: stobs_unsc(LIS_rc%obs_ngrid(k))
  ...
  if(data_update) then 
     ...
     call ESMF_FieldGet(tskinField, localDE=0, farrayPtr=stobs, rc=status)
     call LIS_verify(status)

     if(LIS_rc%obs_ngrid(k).gt.0) then 
        call ESMF_AttributeGet(tskinfield,"Unscaled Obs",stobs_unsc,&
     ...
     if(LIS_masterproc) then 
        ...
        call ISCCP_HXG_stobsname(n,k,obsname)        

        call LIS_create_output_directory('DAOBS')
        open(ftn,file=trim(obsname), form='unformatted')
     endif

     call LIS_writevar_gridded_obs(ftn,n,k,stobs_unsc)
     call LIS_writevar_gridded_obs(ftn,n,k,stobs)
...
end subroutine write_ISCCP_HXG_obs

!BOP
! !ROUTINE: ISCCP_HXG_stobsname
! \label{ISCCP_HXG_stobsname}
! 
! !INTERFACE: 
subroutine ISCCP_HXG_stobsname(n,k,obsname)
! !USES: 
...
end subroutine ISCCP_HXG_stobsname
